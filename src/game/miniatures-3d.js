...
export class MiniatureFactory {
  ...
  createMiniature(unitType, factionId) {
    ...
    const baseRadius = isGOO ? 1.35 : 0.82;
    const baseHeight = isGOO ? 0.4 : 0.25;
    const billHeight = isGOO ? 5.5 : 3.5;
    const billWidth = billHeight * 0.82;

    // 1. Molded 3D Faction Pedestal Base
    const baseGeo = new THREE.CylinderGeometry(baseRadius, baseRadius * 1.15, baseHeight, 36);
    ...
    // Glowing Gold Faction Rim Ring
    const rimMat = new THREE.MeshStandardMaterial({
      color: 0xffd700, // Metallic Gold Rim
      roughness: 0.1,
      metalness: 0.95,
      emissive: hexColor,
    ...
    const rimMesh = new THREE.Mesh(
      new THREE.TorusGeometry(baseRadius * 1.05, 0.07, 8, 36),
      rimMat
    );
    ...
    // Upward Faction Point Light for High-Contrast Glow
    const pLight = new THREE.PointLight(hexColor, isGOO ? 2.5 : 1.8, isGOO ? 10 : 6);
